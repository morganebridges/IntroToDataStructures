...
public class FinalTable
{
   ...
   **/   
   public FinalTable(int capacity)
   {
   ...
   **/
   ...
   /**
    * This findIndex method searches for the index in the keys array that contains a specific key.
    * it is used by the remove, contains, and get methods. 
    * @param key
    * @return - if the key is in the table, it returns the index, otherwise -1
    */
   ...
   /**
    * findIndex2 is used for determining whether the index that a key hashes to is being used, rather
    * than checking for the key itself. This method is used for determining behaviors in the case
    * of a hash collision.
    * @param key
    * @return
    */
   private int findIndex2(Object key)
   // Postcondition: If the specified key is found in the table, then the return
   // value is the index of the specified key. Otherwise, the return value is -1.
   {
      int count = 0;
   ...
   **/
   public void put(Object key, Object element)
   {
      ...
      {  // The key is not yet in this Table.
    	 //System.out.println("This key isn't in table");
         index = hash(key);
         keys[index] = key;
         data[index] = element;
         hasBeenUsed[index] = true;
