...
public class IntBalancedSet2 implements Cloneable
{
   ...
   **/
...

   {
	   ...
	   int i = firstGE(target);
	   if(isLeaf() && data[i] != target)
	   {
		   removed = false;
	   }
	   else if(isLeaf() && data[i] == target )
	   {    
   ...
   //   allowed minimum.
    * 
    * @param i - the index of the subset that will absorb subswet[i + 1]
    */
...

   {
     ...
     if(isLeaf())
     {
    	if(dataCount > 0)
    	{
    		biggestElement = data[dataCount-1];
    	
    	    deleteData(dataCount - 1);
    	}
     }
     else if(subset[childCount-1] != null && subset[childCount-1].dataCount > 0)
     {
